...
var _VueMultiPageWriter = _interopRequireDefault(require("./vue/VueMultiPageWriter"));
  ...
  _createClass(GeneratorFactory, [{
    ...
    value: function create(app, conf) {
      if (conf.type.toLowerCase() === 'vue') {
        ...
        var files = writer.getFiles(vueResult, conf);
        return files;
      }
      ...
      if (conf.type.toLowerCase() === 'html') {
        ...
        var _files = _writer.getFiles(_vueResult, conf);
