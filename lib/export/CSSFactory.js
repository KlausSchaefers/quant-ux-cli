...
function () {
  ...
  _createClass(_default, [{
    ...
    value: function getGridPosition(widget) {
      ...
      if (widget.parent) {
        ...
        if (widget.parent.grid && widget.parent.grid.isRow) {
          ...
          result += "  height: ".concat(this.getResponsiveHeight(widget), ";\n");
          result += "  margin-top: ".concat(this.getResponsiveTop(widget), ";\n");
    ...
    value: function getResponsiveTop(widget) {
    ...
    value: function getResponsiveHeight(widget) {
      return widget.h + 'px';
    }
  }, {
    key: "getGridTracks",
      ...
      if (!Util.isFixedHorizontal(widget)) {
        if (widget.parent) {
          if (Util.isPinnedLeft(widget) && Util.isPinnedRight(widget)) {
            console.debug(' pinned', widget.name, w, widget.parent.w); // result += `  width: 100%;\n`
